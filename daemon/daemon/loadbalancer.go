...
// DeleteSVC deletes the svcKey from the local bpf map.
func (d *Daemon) DeleteSVC(svcKey lbmap.ServiceKey) error {
...
// DeleteRevNAT deletes the revNatKey from the local bpf map.
func (d *Daemon) DeleteRevNAT(revNatKey lbmap.RevNatKey) error {
	d.loadBalancer.BPFMapMU.Lock()
	defer d.loadBalancer.BPFMapMU.Unlock()
	return lbmap.DeleteRevNat(revNatKey)
}

// InsertSVC inserts a service from the given fe (frontend) and bes (backends) with the
// given id. If addRevNAT is set, the RevNAT entry is also created for this particular
// service. If any of the backend addresses set in bes have a different L3 address type
// than the one set in fe, it returns an error without modifying the bpf LB map. If any
// backend entry fails while updating the LB map, the frontend won't be inserted in the
// LB map and therefore there won't be any traffic going to the given backends.
func (d *Daemon) InsertSVC(id types.ServiceID, fe lbmap.ServiceKey, bes []types.L3n4Addr, addRevNAT bool) error {
	d.loadBalancer.BPFMapMU.Lock()
	defer d.loadBalancer.BPFMapMU.Unlock()
